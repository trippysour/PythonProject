...
class Form(QWidget):
    ...
    def search(self):
        ...
        if self.CB_Anim.isChecked():
            anim = self.threadclass.sound_in_anim(self.ln.text(), self.datas)
            if len(anim[0]) != 0:
                self.addTab('AnimationMarkers', anim)

        if self.CB_Particle.isChecked():
            particle = self.threadclass.sound_in_particles(self.ln.text(), self.datas)
            if len(particle[0]) != 0:
                self.addTab('Particles', particle)

        if self.CB_Layers.isChecked():
            layer = self.threadclass.sound_in_lyr(self.ln.text(), self.datas)
            if len(layer[0][0]) != 0:
...
class Progess(QWidget):
    ...
    #
    #     form = QFormLayout()
    #     form.addWidget(self.progressbar)
    #
    #     self.setLayout(form)
    #     self.count = 0
    #
