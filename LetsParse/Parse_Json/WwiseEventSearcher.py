...
GUI 코드
# '''
from PySide2.QtWidgets import QWidget, QVBoxLayout, QMessageBox, QHBoxLayout, QTableWidget, QLineEdit, QPushButton, QApplication, QLabel, QTableWidgetItem, QFileDialog, QStyleFactory, QAbstractItemView
...
class Form(QWidget):
    ...
    #                 pass
        ...
        #         try:
        #             teext = str(self.tb_result.item(currentRow, currentColumn).text())
        #             ws.write(currentRow, currentColumn, teext)
        #         except AttributeError:
        #             pass
        for currentColumn in range(1, self.tb_result.columnCount()):
            for currentRow in range(2, self.tb_result.rowCount()+2):
                try:
                    teext = str(self.tb_result.item(currentRow-1, currentColumn-2).text())
                    ws.cell(currentRow, currentColumn).value = teext
                except AttributeError:
                    pass
